...
![](assets/raspberrypi_sd_cards.jpg)

For this project I will demonstrate two `Linux distributions` so I chose
to label the cards.

![](assets/raspberrypi_sd_cards_labeled.jpg)
...
Find out the devicename of your SD card.
    ...
    root@linux:~# dmesg | tail -2 | cut -c9-
...
the SD card device (without creating the intermediate unzipped file).
    ...
    root@linux:~# unzip -p /tmp/2014-06-20-wheezy-raspbian.zip | dd of=/dev/sdb
...
ethernet cable).
    ...
    root@linux:~# cat /mnt/etc/network/interfaces
    auto lo
...
access to the DHCP leases you can also use that address.
    ...
    student@linux:~$ ping 192.168.42.54
...
Now we login with the user `raspberry` and the password `pi`.
    ...
    student@linux:~$ ssh pi@192.168.42.54
...
![](assets/raspberrypi_camera_connect.jpg)
...
![](assets/raspberrypi_legostack.jpg)
